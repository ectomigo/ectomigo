 ...
 * statement parameters, escape characters, and the like.
 */
function getSqlString(tree, match) {
  let row = match.captures[0].node.startPosition.row;

  const raw = match.captures.reduce((str, c) => {
  ...
  // * :named params
  // * JS ${interpolation}
  // * Python fstring {interpolation}
  return raw.replaceAll('?', '0').replaceAll('%s', '00');
}
