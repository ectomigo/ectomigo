...
export function indexSql(file, code, parentNode) {
  ...
  // TODO *all* columns mentioned in the entire file are attached to each
  // entity reference because columnRefs is a separate query; this will
  // eventually matter with multi-statement scripts e.g. aiosql
...
function getSqlString(tree, match) {
  ...
  return match.captures.reduce((str, c) => {
    ...
    return str + ' '.repeat(c.node.startPosition.column) + getText(tree, c.node)
      ...
      //   for the latter, we'd need to grab the string/stringbuilder etc var
      //   name and make a second pass to get all assignments or additions
